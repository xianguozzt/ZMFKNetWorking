 ...
 *  设置的请求数据类型
 */
static HTRequestType  HT_requestType  = kHTRequestTypeJSON;
/**
 *  监测网络状态
 */
 ...
 *  请求的超时时间
 */
static NSTimeInterval HT_timeout = 15.0f;
/**
 *  是否从从本地提取数据
 */
...
+ (HTURLSessionTask *)deleteWithUrl:(NSString *)url
                       refreshCache:(BOOL)refreshCache
                            success:(HTResponseSuccess)success
                               fail:(HTResponseFail)fail{
    return [self deleteWithUrl:url refreshCache:refreshCache isShowHUD:NO success:success fail:fail];
}
...
+ (void)showHUD:(NSString *)showMessge
{
    ...
    dispatch_main_async_safe(^{
        [ZHShowMessageView showStatusWithMessage:showMessge];
...
{
    dispatch_main_async_safe(^{
        [ZHShowMessageView dismissSuccessView:@"Success"];
...
{
    dispatch_main_async_safe(^{
        [ZHShowMessageView dismissErrorView:@"Error"];
//        [[HTAlertShowView sharedAlertManager] dismissAlertView];
    });
